/**
 * Linux input injection implementation
 * Uses robotjs for cross-platform compatibility
 * Future: Native uinput/XTest for better performance
 */

import { IInputInjector } from './input-injector.js';
import { KeyModifiers } from './input-protocol.js';

export class LinuxInputInjector implements IInputInjector {
  ...
  constructor() {
    try {
      ...
      console.log('[LinuxInjector] robotjs loaded successfully');
    } catch (error) {
      console.warn('[LinuxInjector] robotjs not available - input injection disabled');
    }
  }
  ...
  moveMouse(x: number, y: number): void {
    ...
    try {
      ...
      console.error('[LinuxInjector] Failed to move mouse:', error);
    }
  }
  ...
  mouseDown(button: 'left' | 'right' | 'middle'): void {
    ...
    try {
      ...
      console.error('[LinuxInjector] Failed to press mouse button:', error);
    }
  }
  ...
  mouseUp(button: 'left' | 'right' | 'middle'): void {
    ...
    try {
      ...
      console.error('[LinuxInjector] Failed to release mouse button:', error);
    }
  }
  ...
  mouseClick(button: 'left' | 'right' | 'middle', x?: number, y?: number): void {
    ...
    try {
      ...
      console.error('[LinuxInjector] Failed to click mouse:', error);
    }
  }
  ...
  mouseDoubleClick(button: 'left' | 'right' | 'middle', x?: number, y?: number): void {
    ...
    try {
      ...
      console.error('[LinuxInjector] Failed to double-click mouse:', error);
    }
  }
  ...
  mouseScroll(deltaX: number, deltaY: number): void {
    ...
    try {
      ...
      console.error('[LinuxInjector] Failed to scroll mouse:', error);
    }
  }
  ...
  keyDown(key: string, modifiers?: KeyModifiers): void {
    ...
    try {
      ...
      console.error('[LinuxInjector] Failed to press key:', error);
    }
  }
  ...
  keyUp(key: string, modifiers?: KeyModifiers): void {
    ...
    try {
      ...
      console.error('[LinuxInjector] Failed to release key:', error);
    }
  }
  ...
  keyTap(key: string, modifiers?: KeyModifiers): void {
    ...
    try {
      ...
      console.error('[LinuxInjector] Failed to tap key:', error);
    }
  }
  ...
  typeString(text: string): void {
    ...
    try {
      ...
      console.error('[LinuxInjector] Failed to type string:', error);
    }
  }
    ...
    try {
      switch (combination) {
        ...
        case 'alt-f4':
          ...
          break;
        default:
          console.warn('[LinuxInjector] Unknown special key combination:', combination);
      }
    } catch (error) {
      console.error('[LinuxInjector] Failed to send special key:', error);
    }
  }

  blockInput(): boolean {
    console.warn('[LinuxInjector] Input blocking not supported via robotjs');
  ...
  getMousePosition(): { x: number; y: number } {
    ...
    try {
      ...
      console.error('[LinuxInjector] Failed to get mouse position:', error);
      return { x: 0, y: 0 };
    }
  }
  ...
  private convertModifiersToRobotJS(modifiers?: KeyModifiers): string[] {
    ...
    if (modifiers.meta) result.push('command'); // Super key on Linux
