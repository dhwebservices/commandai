 ...
 * Uses PipeWire (Wayland) or X11 screen capture
 * Future: Native integration with PipeWire for Wayland
 */
export class LinuxScreenCapture implements IScreenCapture {
  ...
  async getDisplays(): Promise<Display[]> {
    ...
    // Future: Query X11/Wayland display information
    return [
      {
        id: 'display-0',
  ...
  async startCapture(displayId: string, quality: CaptureQuality): Promise<void> {
    ...
    console.log('[LinuxCapture] Starting capture:', { displayId, quality });
  ...
  async stopCapture(): Promise<void> {
    ...
    console.log('[LinuxCapture] Stopping capture');
  ...
  private async captureFrame(): Promise<void> {
    ...
    // Future: Actual PipeWire/X11 implementation
