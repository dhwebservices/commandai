# Architecture Decision Records — Index

| ADR | Title | Status |
|---|---|---|
| 001 | Monorepo over polyrepo | Accepted |
| 002 | Rust for the agent core | Accepted |
| 003 | NestJS for the API layer | Accepted |
| 004 | Python/FastAPI for the orchestration layer | Accepted |
| 005 | NATS over Kafka for messaging | Accepted |
| 006 | Action lifecycle state machine | Accepted |
| 007 | gRPC/protobuf for agent<->cloud protocol | Accepted |
| 008 | Explicit SQL migrations over an ORM | Accepted |
| 009 | Supabase for Postgres + Auth | Accepted (founder-directed) |
